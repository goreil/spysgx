...
secret = state.solver.BVS("secret", 8*8)
setattr(state.mem[a_d_addr] ,'unsigned long', secret)
...
traces = [int(x,16) for x in open("traces/libgcrypt/trace.txt").readlines()]
...
# Solution
solution = simgr.active[0].solver.eval(secret)
print("Secret exponent =", hex(solution))